...
**播放**：`space` 播放/暂停，`[` / `]`（或 `ctrl-←` / `ctrl-→`）上一曲 / 下一曲，`,` / `.` 跳转进度，`-` / `=` 音量，`shift-M` 静音，`s` / `r` 随机播放 / 重复，`shift-D` 设备菜单，`shift-L` 全屏歌词，`ctrl-shift-L` 播放栏内歌词，`shift-F` 沉浸视图。
...
- `:tray [on|off]`：桌面应用的关闭按钮是隐藏到托盘还是直接退出（设置中也可选择）。
