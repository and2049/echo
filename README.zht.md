...
- **極速喜歡的歌曲**：採用全域快取架構。您整個喜歡的歌曲庫會快取在本地（`~/.config/echo/liked_songs.json`），實現零延遲的捲動瀏覽，即使有數千首儲存的曲目也毫無壓力。保持同步通常只需一次請求；完整重讀會限速進行、可斷點續傳，並在 Spotify 限流時自動退避。
...
- **快取不同步**：其他裝置上喜歡的歌曲會在啟動時或開啟喜歡的歌曲時出現（最多每 15 分鐘檢查一次）。在其他裝置上取消喜歡的歌曲也會在同一時機被發現，並觸發背景重讀整個庫；被限流的重讀會從中斷處繼續。在 echo 關閉時刪除 `~/.config/echo/liked_songs.json` 可強制完整重讀。
...
- **設定檔路徑**：`~/.config/echo/config.toml`（儲存權杖和偏好設定）、`~/.config/echo/cache.json`（儲存庫快取和喜歡狀態）、`~/.config/echo/liked_songs.json`（儲存喜歡的歌曲列表）、`~/.config/echo/local_library.json` 和 `~/.config/echo/local_playlists.json`。
