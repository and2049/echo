use crate::{
    app::AppState,
    events::AppEvent,
    models::{Track, TrackListContextKind},
};
...
pub fn confirm_duplicate_skip(state: &mut AppState) -> Option<AppEvent> {
    ...
    let tracks: Vec<_> = prompt
        ...
        .collect();
    (!tracks.is_empty()).then_some(AppEvent::AddTracksToPlaylist(prompt.playlist_id, tracks))
}
    ...
    match c.kind {
        TrackListContextKind::Playlist if c.id != "LIKED_SONGS" => {
            if owns_playlist(state, &c.id) {
                vec![Edit, Queue, CopyLink, Delete]
            } else {
                vec![ToggleSaved, Queue, CopyLink]
            }
        }
        TrackListContextKind::Album => vec![ToggleSaved, Queue, CopyLink],
        TrackListContextKind::LocalPlaylist => vec![Rename, Delete],
        _ => Vec::new(),
    }
}
...
mod tests {
    ...
    fn page_actions_distinguish_ownership_albums_and_local_playlists() {
        ...
        assert_eq!(
            ...
            [Edit, Queue, CopyLink, Delete]
        );
        assert!(toggle_follow_playlist(&state, "p").is_none());
        state.data.user_id = Some("other".into());
        assert_eq!(
            playlist_page_actions(&state),
            [ToggleSaved, Queue, CopyLink]
        );
        assert!(matches!(
        ...
        assert_eq!(playlist_page_actions(&state), [Rename, Delete]);
