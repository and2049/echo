...
use echo_core::models::TrackListContext;
...
fn open_spotify_target(state: &mut AppState, target: SpotifyTarget) -> Option<AppEvent> {
    match target {
        SpotifyTarget::Track(track_id) => Some(AppEvent::PlayTrack {
            target: echo_core::models::PlaybackTarget::SpotifyTrack {
            ...
            if let Some(cmd_name) = args.next() {
                match cmd_name {
                    ...
                    "open" => {
                        let value = {
                            ...
                            if remainder.is_empty() {
                                match echo_core::platform::read_clipboard() {
                                    Ok(value) => value,
                                    Err(error) => {
                                        set_status(
                                            state,
                    ...
                    "newfolder" => {
                        ...
                        if !name.is_empty() {
                            state.ui.library_config.folders.push(echo_core::config::Folder {
                                name,
                                is_open: true,
                                playlists: vec![],
                            });
                            ...
                            match mode {
                                "alpha" => {
                                    state.ui.library_config.sort_mode =
                                        echo_core::config::SortMode::Alphabetical
                                }
                                "creator" => {
                                    state.ui.library_config.sort_mode =
                                        echo_core::config::SortMode::Creator
                                }
                                ...
                                | "added" | "reverse" => {
                                    if state.ui.active_view != echo_core::app::ActiveView::TrackList {
                                        ...
                                        let sort = match mode {
                                            "title" => echo_core::app::TrackSort::Title,
                                            "artist" => echo_core::app::TrackSort::Artist,
                                            "album" => echo_core::app::TrackSort::Album,
                                            "duration" => echo_core::app::TrackSort::Duration,
                                            "added" => echo_core::app::TrackSort::Added,
                                            _ => echo_core::app::TrackSort::Original,
                                        };
                                        if mode == "reverse" {
                    ...
                    "delfolder" => {
                        ...
                        if state.ui.active_view == echo_core::app::ActiveView::Library
                            && state.ui.selected_playlist_index < state.data.library_view.len()
                            && let echo_core::models::LibraryNode::Folder(f) =
                    ...
                    "pixelate" => {
                        if let Some(pixel_str) = args.next() {
                            if let Ok(pixels) = pixel_str.parse::<u32>() {
                                ...
                                if state.ui.active_view == echo_core::app::ActiveView::TrackList {
                                    return Some(echo_core::events::AppEvent::ReloadHeaderImage);
                                }
                            } else {
                                state.ui.status_message =
                    ...
                    "rename" => {
                        let name = args.collect::<Vec<&str>>().join(" ");
                        if !name.is_empty()
                            && state.ui.active_view == echo_core::app::ActiveView::Library
                            && let Some(node) = state
                                .data
                                .library_view
                    ...
                    "queue" => {
                        state.ui.active_view = echo_core::app::ActiveView::Queue;
                        state.ui.selected_queue_index = 0;
                        return Some(echo_core::events::AppEvent::FetchQueue);
                    }
                    "vis" => {
