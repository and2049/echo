...
use echo_core::models::{ActionMenuAction, Playlist, SearchTrack, Track, TrackListContext};
...
fn handle_action_menu_action(
    ...
    match action {
        ActionMenuAction::GoToAlbum => {
            if ctx.source == echo_core::models::TrackSource::Local {
                ...
                return Some(AppEvent::LoadContextTracks(
                    echo_core::models::TrackListContext::album(
                        album_id,
                        String::new(),
                        String::new(),
                        None,
        ...
        ActionMenuAction::GoToArtist => {
            if ctx.source == echo_core::models::TrackSource::Local && !ctx.artist_name.is_empty() {
        ...
        ActionMenuAction::CopyLink => {
            match echo_core::platform::copy_to_clipboard(&format!(
        ...
        ActionMenuAction::CopyPath => {
            if let Some(path) = ctx.local_path {
                match echo_core::platform::copy_to_clipboard(&path.to_string_lossy()) {
        ...
        ActionMenuAction::OpenFolder => {
            if let Some(path) = ctx.local_path
                && let Err(error) = echo_core::platform::reveal_file(&path)
            {
