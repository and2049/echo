...
pub fn render_playback_bar(frame: &mut Frame, state: &mut AppState, area: Rect) {
    let shuffle_str = if state.playback.is_shuffled {
        echo_core::i18n::t("ui.on", &state.ui.library_config.language)
    } else {
        echo_core::i18n::t("ui.off", &state.ui.library_config.language)
    };
    ...
    let mut border_title = echo_core::i18n::t("ui.playing", &state.ui.library_config.language);
