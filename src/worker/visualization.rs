...
impl VisualizationSink {
    pub fn new(inner: Box<dyn Sink>, shared_bands: Arc<Mutex<[f32; BANDS]>>, enable_flag: Arc<std::sync::atomic::AtomicBool>) -> Self {
    ...
    fn compute_fft(&mut self) {
        ...
        for i in 0..BANDS {
            let mag = bands[i];
            let db = if mag > 0.001 { 20.0 * mag.log10() } else { -60.0 };
